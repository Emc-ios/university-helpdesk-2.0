...
class _FaqManagementScreenState extends State<FaqManagementScreen> {
                      ...
                      if (data['imageUrl'] != null)
                        Padding(
                          ...
                          child: Row(children: [
                            ...
                            Expanded(child: Text("Image: ${data['imageUrl']}", style: const TextStyle(color: Colors.blue, fontSize: 10), overflow: TextOverflow.ellipsis))
                          ]),
                        ),
                    ],
                  ),
  ...
  void _showFaqDialog({String? docId, Map<String, dynamic>? existingData}) {
    ...
    final _answerController = TextEditingController(text: existingData?['answer'] ?? '');
    final _keywordsController = TextEditingController(
      text: existingData != null && existingData['keywords'] is List 
          ? (existingData['keywords'] as List).join(', ') 
          : ''
    );
                  ...
                  Row(
                    children: [
                      ...
                      if (_selectedImage != null)
                        ...
                        const Text("Saved", style: TextStyle(color: Colors.blue, fontSize: 12)),
                    ],
                  ),
                  if (_isUploading) const LinearProgressIndicator(),
                ],
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: _isUploading ? null : () async {
                  ...
                  String? imageUrl = _existingImageUrl;
                  if (_selectedImage != null) {
                    imageUrl = await _dbService.uploadFile(_selectedImage!);
                  }
