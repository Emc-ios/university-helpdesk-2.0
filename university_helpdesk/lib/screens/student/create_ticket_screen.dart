...
class _CreateTicketScreenState extends State<CreateTicketScreen> {
  ...
  final List<String> _categories = [
    'Enrollment', 'Scholarship', 'Grades & Records', 'Account Issue', 'Other'
  ];
      ...
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.stretch,
            children: [
              const Text("Submit an inquiry to the University Office.", style: TextStyle(fontSize: 16, color: Colors.grey)),
              const SizedBox(height: 20),
              ...
              OutlinedButton.icon(
                ...
                icon: const Icon(Icons.attach_file),
                label: Text(_attachedImage != null ? "Image Attached" : "Attach Image (Optional)"),
              ),
              if (_attachedImage != null)
                Padding(
                  padding: const EdgeInsets.only(top: 5),
  ...
  void _submitTicket() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        String? attachmentUrl;
        if (_attachedImage != null) {
          attachmentUrl = await _dbService.uploadFile(_attachedImage!);
        }
        ...
        if (mounted) {
          showDialog(
            ...
            builder: (ctx) => AlertDialog(
              title: const Text("Success"),
              content: const Text("Your ticket has been submitted."),
              actions: [
                TextButton(
                  onPressed: () {
                    Navigator.pop(ctx);
